...
#' the Climate Hazards Center InfraRed Precipitation with Station data ([CHIRPS](https://www.chc.ucsb.edu/data)) via the [`chirps`](https://docs.ropensci.org/chirps/) R package.
#' Additionally, the optional `intervals` argument specifies a set of intervals over which the function will calculate the
#' cumulative sum of precipitation in millimeters (mm) for the previous X number of days for each location.
#'
...
get_precip_data <- function(lon,
     ...
     message("Downloading CHIRPS precipitation data via 'chirps' package")
