...
While absolute quantification of the number of target gene copies is nicely interpretable, it does not explicitly account for the starting amount of nucleic acid in the sample or amplification efficiency of the PCR assay [ref]. Normalization of qPCR data has been a general methodological challenge in biology and has motivated much work on statistical methods to transform qPCR data [ref, ref]. One method of normalization is the $\Delta \Delta Ct$ method of calculating the fold change in gene expression of the target relative to one or more reference targets [ref]. The $\Delta \Delta Ct$ method is perhaps the most robust way to normalize qPCR data and which is particularly important for environmental sampling data due to the often uncontrolled sources of samples.

Here we have developed functions that allow for the calculation of the standard $\Delta \Delta Ct$ described by @livak2001 and the adjusted $\Delta \Delta Ct$, which is adjusted for the PCR amplification efficiency, described by @yuan2008. We have also included functionality to estimate the percentile amplification efficiency for a target nucleic acid following the methods in [@yuan2008; @yuan2006]. 

The code below shows how to calculate both the standard and adjusted $\Delta \Delta Ct$ values for a simulated environmental sampling data set shown in the `template_es_data` and `template_standard_curve` data objects.
